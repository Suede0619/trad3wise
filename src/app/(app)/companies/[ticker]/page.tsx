...
import { getCompany, getPriceSeries, listFilings, listInsiderTransactions } from "@/lib/data";
...
export default async function CompanyOverview({ params }: { params: Promise<{ ticker: string }> }) {
  ...
  const filings = listFilings().filter((f) => f.ticker === company.ticker).slice(0, 5);
        ...
        <Card>
          ...
          <CardContent className="space-y-1">
            {filings.length ? filings.map((f) => (
              <Link key={f.id} href={`/sec-filings/${f.id}`} className="flex items-center justify-between gap-2 rounded-md px-2 py-1.5 hover:bg-surface-2">
